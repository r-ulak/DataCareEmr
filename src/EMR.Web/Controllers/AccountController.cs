...
using EMR.Users;
using EMR.Web.Controllers.Results;
using EMR.Web.Models.Account;
...
{
    public class AccountController : EMRControllerBase
    {
        ...
        public AccountController(
            ...
            ITenantAppService tenantAppService, ISmtpEmailSender emailSender,
        ...
        public async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "")
        {
            ...
            {
                await PrepareAndSendConfirmEmail(loginResult.User, loginResult.User.EmailConfirmationCode);
                throw CreateExceptionForFailedLoginAttempt(AbpLoginResultType.UserEmailIsNotConfirmed,
                    loginModel.UsernameOrEmailAddress, loginModel.TenancyName);
            }
            if (string.IsNullOrWhiteSpace(returnUrl))
            {
                returnUrl = Request.ApplicationPath;
        ...
        private Exception CreateExceptionForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName)
        {
            switch (result)
            {
                ...
                case AbpLoginResultType.UserEmailIsNotConfirmed:
                    return new UserFriendlyException(L("LoginFailed"), "Your email address is not confirmed. You can not login. Another Confirmation has been sent to you"); //TODO: localize message
        ...
        public async Task<ActionResult> ConfirmEmail(string Id, string code)
        {
            ...
            var result = await _tenantAppService.VerifyEmail(Convert.ToInt64(Id), code);
        ...
        {
            return View("RegisterTenant", new CreateTenantInput());
        }
        ...
        [AllowAnonymous]
        [HttpPost]
        public async Task<ActionResult> SendAccountConfirmEmail(string Id)
        {
            if (Id.IsNullOrEmpty())
            {
                return View("Error");
            }
            var user = await _userAppService.GetUserAsync(Convert.ToInt32(Id));
            string code = user.EmailConfirmationCode;
            await PrepareAndSendConfirmEmail(user, code);
            return View("DisplayEmail");
        }

        private async Task PrepareAndSendConfirmEmail(User user, string code)
        {
            var callbackUrl = Url.Action("ConfirmEmail", "Account", new { Id = user.Id, code = code }, protocol: Request.Url.Scheme);
            StringBuilder emailBody = new StringBuilder(AppSettings.ConfirmEmailTemplate);
            emailBody.Replace(":FullName", user.Name);
            ...
            mailMsg.To.Add(new MailAddress(user.EmailAddress, user.Name));
            await _emailSender.SendAsync(mailMsg, true);
        }

        [HttpPost]
        [AllowAnonymous]
        [UnitOfWork]
        public virtual async Task<ActionResult> RegisterTenant(CreateTenantInput model)
        {
            try
            {
                CheckModelState();
                var tenantAdmin = await _tenantAppService.CreateTenant(model);
                //Login!
                var loginResult = await GetLoginResultAsync(model.AdminEmailAddress, model.Password, model.TenancyName);
                if (loginResult.Result == AbpLoginResultType.Success)
                {
                    await PrepareAndSendConfirmEmail(loginResult.User, tenantAdmin.EmailConfirmationCode);
